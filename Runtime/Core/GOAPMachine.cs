...
namespace CZToolKit.GOAP_Raw
{
    public class GOAPMachine
    {
        ...
        public void Plan(GOAPGoal goal, int maxDepth, ref Queue<GOAPAction> plan)
        {
            ...
            foreach (var node in treeRoot.Enumerate())
            {
        ...
        /// <param name="maxDepth">已找到的所有计划</param>
        ...
        private GOAPNode BuildGraph(GOAPGoal goal, int maxDepth)
        {
            ...
            void InnerBuilderGraph(GOAPNode parent, int depth)
            {
                ...
                foreach (var action in actions)
                {
                    ...
                    // 不允许出现两个连续的相同行为
                    if (parent == null || action == parent.action || !action.IsProceduralPrecondition(parent.state))
                        continue;
        ...
        public static bool IsAchieve(Dictionary<string, bool> l, Dictionary<string, bool> r)
        {
            ...
            {
                if (!l.TryGetValue(pair.Key, out bool value))
                    return false;
                if (!pair.Value == value)
                    return false;
            }
    ...
    public class GOAPNode
    {
        ...
        public IEnumerable<GOAPNode> Enumerate()
        {
            foreach (var item in InnerEnumerate(this))
            {
                yield return item;
            }

            IEnumerable<GOAPNode> InnerEnumerate(GOAPNode parent)
            {
                yield return parent;
                foreach (var child in parent.children)
                {
                    foreach (var item in InnerEnumerate(child))
                    {
                        yield return item;
                    }
                }
